...
import com.indianstudygroup.databinding.ActivityGymScheduleDetailsBinding
...
class GymScheduleDetailsActivity : AppCompatActivity() {
    private lateinit var binding: ActivityGymScheduleDetailsBinding
    private lateinit var scheduleData: GymScheduleResponseModelItem

    @RequiresApi(Build.VERSION_CODES.O)
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = ActivityGymScheduleDetailsBinding.inflate(layoutInflater)
        ...
        if (receivedIntent.hasExtra("ScheduleData")) {
            val userDetails: GymScheduleResponseModelItem? =
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == 1) {
            if (resultCode == RESULT_OK) {
                val libraryDataPhoto = data?.getStringExtra("DataPhoto")
                val libraryDataName = data?.getStringExtra("DataName")
                val libraryDataAddress = data?.getStringExtra("DataAddress")
                val libraryDataTime = data?.getStringExtra("DataTime")
                val libraryRating = data?.getStringExtra("Rating")
                val libraryReview = data?.getStringExtra("Review")
                showStartDialog(
